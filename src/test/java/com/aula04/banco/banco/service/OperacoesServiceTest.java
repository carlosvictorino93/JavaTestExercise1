...
public class OperacoesServiceTest {
    ...
    public void deveSacar(){
        ...
        requestSaque.setConta(conta.getId());
        requestSaque.setValor(100.00);

        try {
            operacoesService.depositar(cliente.getId(), requestDeposito);
            operacoesService.sacar(cliente.getId(), requestSaque);
            Assertions.assertTrue((saldo+requestDeposito.getValor()) - requestSaque.getValor() == conta.getSaldo());
        } catch (Exception e ) {
            fail("Lancou a excecao");
        }
    }
    ...
    public void naoDeveSacar(){
        ...
        RequestSaque requestSaque = new RequestSaque();
        requestSaque.setConta(contaId);
        requestSaque.setValor(1000000.00);
        try {
            operacoesService.sacar(id, requestSaque);
